...
function ruleSummary(state: RoomState): string {
  ...
  switch (r) {
    case "mon_batsu": return `${state.winCount}○${state.eliminateCount}× ／ ${t}`;
    case "mon_kyu":   return `${state.winCount}○${state.eliminateCount}休 ／ ${t}`;
    case "nbn":       return `${state.nbyN}byN ／ ${t}`;
    case "points":    return `+${state.addPoints}/-${state.subtractPoints} ${state.winPoints}pt ／ ${t}`;
    default:          return `シンプル ／ ${t}`;
  }
}
  ...
  return (
    <div className="space-y-6 text-white">
      <div>
        ...
        <p className="text-gray-400 text-sm">ルームID: <span className="font-mono">{roomId}</span></p>
      </div>
