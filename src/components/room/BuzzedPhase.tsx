  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8 text-white">
      ...
      <div className="w-full max-w-md space-y-2">
        {buzzes.map((buzz, i) => {
          ...
          return (
            ...
            >
              <span className={`w-6 text-center font-black ${isFirst ? "text-black" : "text-gray-400"}`}>
                {i + 1}
              </span>
              <span className="flex-1 font-bold">
                {player?.name ?? "?"}
                {isMe && <span className="ml-2 text-sm opacity-75">（あなた）</span>}
              </span>
              <span className={`text-sm font-mono ${isFirst ? "text-black/70" : "text-gray-400"}`}>
      ...
      <p className="text-gray-500 text-xs">
        判定対象: <span className="text-yellow-400">{players[buzzes[0]?.playerId]?.name}</span>
      </p>
    </div>
  );
}
